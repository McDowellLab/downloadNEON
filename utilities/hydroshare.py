...
def check_for_ipynb(content_files):
    ...
    for f, p in content_files.items():
        if f[-5:] == 'ipynb':
            ...
            url = '%s%s/notebooks/notebooks/%s' % (
            ':'.join(os.environ['JUPYTER_HUB_IP'].split(':')[:-1]), os.environ['JPY_BASE_URL'], rel_path)
...
class ResourceMetadata(object):
    ...
    def parse_science_metadata(self):
        ...
        search = '{{{0}}}Description/{{{1}}}description/{{{0}}}Description/{{{2}}}abstract'.format(self.ns['rdf'],
                                                                                                   self.ns['dc'],
                                                                                                   self.ns['ns2'])
